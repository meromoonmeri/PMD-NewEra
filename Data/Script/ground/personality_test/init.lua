        ...
        if CONFIG.RegularStarters then
                ...
                while result.Species == hero_choice.Species or not continue do --do not allow same species for partner and player
                        ...
                        if result.Species == hero_choice.Species then
                                UI:WaitShowDialogue("Le joueur et le partenaire ne peuvent pas être de la même espèce.[pause=0] Veuillez choisir à nouveau.")
                        else
                                ...
                                if hero_type == partner_type then --warn player if types match
                                        UI:WaitShowDialogue("Attention ![pause=0] Votre héros et votre partenaire partagent le même type principal.")
                                        UI:WaitShowDialogue("Cela pourrait rendre votre aventure plus difficile que d'ordinaire.")
                ...
                for charslot = 0, 1, 1 do 
                        if charslot == 1 then
                                GAME:WaitFrames(20)
                                UI:WaitShowDialogue("Et maintenant,[pause=10] votre partenaire.")
                        end
                        ...
                        while not animation_continue do
                                ...
                                while not species_continue do
                                        ...
                                        while not legit_species do
                                                ...
                                                if not legit_species then 
                                                        UI:WaitShowDialogue(result .. " n'est pas une espèce valide.[pause=0] Veuillez réessayer.")
                                                else
                                                        if not _DATA:GetMonster(lower_result).Released then 
                                                                UI:WaitShowDialogue(_DATA:GetMonster(lower_result):GetColoredName() .. " ne possède pas de données et/ou de sprites complets.[pause=0] Veuillez réessayer.")
                                                                UI:WaitShowDialogue("Si vous pensez qu'il s'agit d'une erreur,[pause=10] veuillez vérifier le dépôt de sprites avant d'en informer le développeur.")
                                                                legit_species = false
                                                        else
                                ...
                                if  _DATA:GetMonster(char_choice.Species).Forms.Count > 1 then 
                                        ...
                                        while not form_continue do
                                                ...
                                                if not _DATA:GetMonster(char_choice.Species).Forms[form].Released then
                                                        UI:WaitShowDialogue("Cette forme ne dispose pas des données ou sprites nécessaires.[pause=0] Veuillez réessayer.")
                                                        UI:WaitShowDialogue("Si vous pensez qu'il s'agit d'une erreur,[pause=10] veuillez vérifier le dépôt de sprites avant d'en informer le développeur.")
                                                else
                                ...
                                if missing_animations then 
                                        UI:WaitShowDialogue("ATTENTION ![pause=0] L'espèce et la forme que vous avez choisies manquent de certaines animations de starter utilisées dans les cinématiques.")
                                        UI:WaitShowDialogue("Le jeu devrait fonctionner correctement,[pause=10] mais certaines cinématiques et expressions de portrait peuvent être décalées en conséquence.")
