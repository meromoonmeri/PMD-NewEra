...
function guild_heros_room.Book_Action(obj, activator)
    ...
    GeneralFunctions.TurnTowardsLocation(partner_ground, obj.Position.X + obj.Width // 2, obj.Position.Y + obj.Height // 2)
        ...
        local choices = {"Changer les écharpes", "Changer les surnoms", "Changer le nom d'équipe", "Vérifier le rang", "Rien"}
        UI:BeginChoiceMenu("Que souhaitez-vous faire ?", choices, 1, #choices)
        UI:WaitForChoice()
        ...
        if choice_result == 1 then
                local state = 1
                while state > 0 do
                        UI:BeginChoiceMenu("De qui souhaitez-vous changer l'écharpe ?", {hero_ground:GetDisplayName(), partner_ground:GetDisplayName(), "Aucun"}, 1, 3)
                        UI:WaitForChoice()
                        ...
                        if fallback.Skin == monId:ToCharID().Skin then                  
                                UI:BeginChoiceMenu("Quelle couleur souhaitez-vous ?", {"Noir", "Bleu", "Cyan", "Vert", "Magenta", "Orange", "Rose", "Violet", "Rouge", "Blanc", "Jaune", "Pas d'écharpe"}, 1, 12)
                                UI:WaitForChoice()
                                ...
                                if new_skin ~= current_skin then 
                                        ...
                                        GAME:WaitFrames(40)
                                        if chara_choice == 1 then
                                                GeneralFunctions.HeroDialogue(chara, "(Pas mal !)", "Normal")
                                        else
                                                UI:SetSpeaker(chara)
                                                UI:WaitShowDialogue("Pas mal !")
                                        end
                                        GAME:WaitFrames(20)
                                end
                                ...
                                UI:WaitShowDialogue("Il n'y a pas encore de sprites d'écharpe pour ce Pokémon.[pause=0] Revenez vérifier plus tard !")
                                state = -1
                        end
        ...
        elseif choice_result == 2 then
                UI:BeginChoiceMenu("De qui souhaitez-vous changer le nom ?", {hero_ground:GetDisplayName(), partner_ground:GetDisplayName(), "Aucun"}, 1, 3)
                UI:WaitForChoice()
                local name_result = UI:ChoiceResult()

                if name_result == 1 then
                        local name = hero.Nickname
                        UI:NameMenu("Comment vous appelez-vous ?", "Appuyez sur [ESC] pour garder l'ancien surnom.", 60)
                        UI:WaitForChoice()
                        local result = UI:ChoiceResult()
                        --if no name given, set name to previous name
                        if result == "" then result = name end
                        GAME:SetCharacterNickname(hero, result)
                        UI:WaitShowDialogue("Vous vous appelez désormais " .. hero_ground:GetDisplayName() .. " !")
                elseif name_result == 2 then
                        local name = partner.Nickname
                        UI:NameMenu("Comment s'appelle votre partenaire ?", "Appuyez sur [ESC] pour garder l'ancien surnom.", 60)
                        UI:WaitForChoice()
                        local result = UI:ChoiceResult()
                        --if no name given, set name to previous name
                        if result == "" then result = name end
                        GAME:SetCharacterNickname(partner, result)
                        UI:WaitShowDialogue("Votre partenaire s'appelle désormais " .. partner_ground:GetDisplayName() .. " !")
                end
                
        elseif choice_result == 3 then
                local name = _DATA.Save.ActiveTeam.Name
                UI:NameMenu("Comment s'appelle votre équipe ?", "Appuyez sur [ESC] pour garder l'ancien nom d'équipe.", 60)
                UI:WaitForChoice()
                local result = UI:ChoiceResult()
                --if no name given, set name to previous name
                if result == "" then result = name end
                GAME:SetTeamName(result)
                UI:WaitShowDialogue("Votre équipe s'appelle désormais Équipe " .. GAME:GetTeamName() .. " !")
        elseif choice_result == 4 then
                ...
                UI:WaitShowDialogue("Votre équipe est actuellement au rang [color=#FFA5FF]" .. current_rank:gsub("^%l", string.upper) .. "[color].")
                UI:WaitShowDialogue("Il vous faut [color=#00FFFF]" .. to_go .. "[color] Points de Rang Aventurier pour passer au rang [color=#FFA5FF]" .. next_rank:gsub("^%l", string.upper) .. "[color].")
        end
