...
function metano_inn_ch_4.Passerby_1_Action(chara, activator)
        if not SV.Chapter4.FinishedGrove then 
                ...
                UI:WaitShowDialogue("Je ne voudrais pas payer de frais supplémentaires si j'ai laissé une marque sur le lit par accident...")
        else 
...
function metano_inn_ch_4.Nidorina_Action(chara, activator)
        if not SV.Chapter4.FinishedGrove then
                ...
                UI:WaitShowDialogue("Je veux pas travailler dans cette auberge stupide ![pause=0] Mais ils comprennent pas ça,[pause=10] bien sûr que non !")
        end
        GeneralFunctions.EndConversation(chara)
end
...
function metano_inn_ch_4.Innkeeper_Desk_Right_Action(chara, activator) 
        if not SV.Chapter4.FinishedGrove then
                ...
                UI:WaitShowDialogue("Il aurait déjà dû prononcer son premier mot maintenant...[pause=0] J'espère qu'il le fera très bientôt !")
...
function metano_inn_ch_4.Innkeeper_Desk_Left_Action(chara, activator) 
        if not SV.Chapter4.FinishedGrove then
                ...
                UI:WaitShowDialogue("J'imagine qu'il y a moins de Pokémon sur les routes à cause des donjons mystères et des hors-la-loi et tout ça...")
